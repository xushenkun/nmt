...
if [ "$use_bpe" = "1" ]; then
  ...
  cat "${corpus_home}/train.tc.zh" | python3 ${current_dir}/vocab.py ${vocab_min_freq} >> "${corpus_home}/vocab.zh"
  echo -e "<unk>\n<s>\n</s>" > "${corpus_home}/vocab.en"
  cat "${corpus_home}/train.tc.en" | python3 ${current_dir}/vocab.py ${vocab_min_freq} >> "${corpus_home}/vocab.en"
fi
